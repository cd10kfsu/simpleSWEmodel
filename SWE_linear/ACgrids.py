...
def SWE_Cgrid(SWEparams, grid, dt, nt, u, v, h, h0):
    ...
    "Run for nt time steps of length dt with orography h0"
    ...
    fu = SWEparams['beta']*grid.yu
    fv = SWEparams['beta']*grid.yv
    ...
    for it in xrange(nt):
        ...
        for iu in xrange(2):
            if (iu+it)%2 == 0:
                u = uOld + dt*(fu*vatu(v) - g*ddxC(h+h0, grid))
                
            else:
                v = vOld + dt*(-fv*uatv(u) - g*ddyC(h+h0, grid))
